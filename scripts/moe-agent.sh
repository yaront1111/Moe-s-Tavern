...
if [ "$CLI_TYPE" = "codex" ]; then
    ...
    if [ -n "$TOML_PROXY_CMD" ]; then
        $PYTHON_CMD - "$CODEX_CONFIG_FILE" "$TOML_PROXY_CMD" "$TOML_PROXY_ARGS" "$PROJECT" << 'PYEOF'
import sys, os
...
if os.path.exists(config_file):
    # Merge: read existing, remove old moe sections, append new
    with open(config_file, "r") as f:
        lines = f.readlines()
...
$TEAM_CONTEXT"
    ...
    if [ "$CLI_TYPE" = "codex" ]; then
        ...
        if [ "$CODEX_EXEC" = true ]; then
            ...
            $COMMAND exec -C "$PROJECT" --full-auto --sandbox workspace-write "$FULL_PROMPT" || true
        else
