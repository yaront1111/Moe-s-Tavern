...
- **Nothing else closes it on a worker's behalf.** Worker deletion, the startup purge and the sweeps select tasks by assignment, so they do not reach a finalizing attempt, and none of them may close one on an idle signal. A runner killed with no exit trap leaves the row held until a governor or human closes the boundary (see `moe.finalize_attempt`'s **Escape for a runner that is gone**).
...
- **Finalizing hold (a hard refusal, unlike the commit gate).** While the task has an execution attempt in the `finalizing` phase, approval is refused with `-32002` / `ATTEMPT_FINALIZING` before any mutation — no DONE write, no worker touch, no chat line, a byte-identical task file. The bytes are not landed yet, so DONE would be premature. The hold is scoped **by task, not by worker**, because the IDE/human approval path carries no `workerId` at all and the REVIEW handoff has already cleared `assignedWorkerId`. It is lifted by `moe.finalize_attempt`, or by its runner's `moe.deregister_worker` (which is not evidence of a landing — see that tool), and its `context.retryable` is `true` — retry after the runner finalizes rather than escalating. A QA claim can no longer lift it: while the attempt is `finalizing`, no seat other than its own worker may claim the row (see `moe.claim_next_task`'s **Finalizing hold on the task**). A claim used to close the attempt, which silently dropped this hold and fenced the runner's `moe.record_candidate` and `moe.finalize_attempt` out as superseded. See `moe.finalize_attempt`.
