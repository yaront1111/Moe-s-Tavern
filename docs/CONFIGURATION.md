...
| `attribution.contested` | A path this task asserted that another live task also declares: `skip-untouched` lands it only when this session's editing tools wrote it (otherwise `MOE_ATTR_CONTESTED_UNTOUCHED(task-<peer>)` — an untouched file carries the peer's unlanded hunks); `commit` lands it regardless and records `Moe-Contested: <path> (task-<peer>)` in the commit body; `skip` drops it with `MOE_ATTR_CONTESTED`. Independently of this policy, a landing source file whose relative import resolves neither in HEAD nor in the same commit is held back with `MOE_ATTR_IMPORTEE_MISSING(<specifier>)` | `skip-untouched` (default) / `commit` / `skip` |
