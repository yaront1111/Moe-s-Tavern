...
| `MOE_CODEX_SANDBOX` | `--sandbox <mode>` for the headless `codex exec` launch (worker/qa default, `-CodexExec`/`--codex-exec`): `read-only`, `workspace-write` or `danger-full-access`; `inherit` omits the flag so the merged `~/.codex/config.toml` + `<project>/.codex/config.toml` `sandbox_mode` decides — the wrapper writes none, so with neither set codex runs **read-only**. On Windows codex enforces `workspace-write` only when `~/.codex/config.toml` has `[windows] sandbox = "unelevated"` (or `"elevated"`); without it `exec` silently runs read-only and a headless worker cannot write files — use `danger-full-access` there. Approval stays `never` in `exec`: the wrapper also pins `-c approvals_reviewer=user` on the headless seat (an `approvals_reviewer = "auto_review"` in the operator's config would otherwise drop that override, as `--full-auto` used to prevent). The wrapper never passes `--full-auto` — codex-cli 0.147+ rejects it (exit 2 at launch, relaunch loop); an unknown value warns and falls back to the default | `workspace-write` | `danger-full-access` |
