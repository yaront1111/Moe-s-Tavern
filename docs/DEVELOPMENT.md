...
- `-CodexExec` / `--codex-exec`: Force the `codex` CLI headless (`codex exec`) for a seat. Codex seats run the interactive TUI for every role, so this flag is the only route to headless — and the only way a codex seat lands a commit per task, since the wrapper's post-flight runs after the CLI exits and a TUI seat exits only when the operator closes it. The sandbox for that headless launch comes from `MOE_CODEX_SANDBOX` (modes, default and the measured reasons behind it in [CONFIGURATION.md](./CONFIGURATION.md#agent-scripts)). If the installed codex rejects the wrapper's launch argv, the seat stops with `MOE_CLI_ARGV_REJECTED` plus a `@governors` line in `#general` and deregisters (the task returns to the queue) instead of relaunch-looping; `MOE_DISABLE_ARGV_PROBE=1` skips that probe
...
4. **Branch**: `Ensure-MoeSafeBranch` / `ensure_safe_branch` — never `main`/`master`/detached/unborn; peel onto a literal `settings.consolidationBranch` else `moe/work-<yyyy-MM-dd>`; a literal `main`/`master` that the tree is already on means main-direct: stay, no checkout — failure → rescue ref `[reason=peel-failed]`, `break` in both wrappers.
