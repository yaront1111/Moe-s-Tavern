...
- Worker post-flight auto-commits **and pushes** when a task lands in `REVIEW` (gated by `.moe/project.json` `settings.autoCommit`, default on). If `settings.qualityGate` is set (a shell command, e.g. lint+typecheck), the wrapper runs it first — but only on the epic's FINAL task (highest `order` among siblings; `settings.qualityGateScope: 'epicFinal'` default, `'everyTask'` opt-in) so mid-epic tasks stay lean: non-zero exit skips commit+push, posts a PUSH-BLOCKED chat message + the output tail as a task comment, and hard-stops the worker loop (so the next task's `git add -A` can't absorb the edits); `MOE_DISABLE_QUALITY_GATE=1` skips per-run. Branch safety: it refuses to commit on `main`/`master`, peeling onto a shared `moe/work-<YYYY-MM-DD>` branch first (not branch-per-task; existing non-default branches are reused).
- Approval modes (`CONTROL` default / `SPEED` / `TURBO`), `agentCommand`, `autoCommit`, `qualityGate`, `taskSizing`, `models.{role}`, and `enableAgentTeams` live in `.moe/project.json` `settings`.
- **Task-size + verification gates** (both tunable/removable only in code or settings — they exist because prompt-only rules measurably don't hold): `submit_plan` warns past 8 steps / 5 distinct affectedFiles and hard-rejects past 12 / 10 (`settings.taskSizing`; logic in `util/planSize.ts`) with "split via moe-epic-breakdown/SPIDR" guidance; `complete_task` requires `verification: { command, exitCode: 0, outputTail? }`, persisted on the task and surfaced to QA via `get_context` for re-run auditing; `qa_approve` requires a `summary` of what was verified (persisted as `reviewSummary`). `scripts/analyze-task-metrics.mjs` reports reopen/reject rates per plan-size bucket from `.moe/tasks/*.json` for tuning the thresholds.
